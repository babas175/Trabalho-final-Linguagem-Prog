...
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Exp","Type","num","'+'","'*'","'-'","\"&&\"","\"||\"","\"==\"","\">\"","\">=\"","true","false","if","then","else","var","'\\\\'","':'","\"->\"","'('","')'","Bool","Number","'!'","','","'pr'","'sg'","let","'='","in","%eof"]
        bit_start = st * 35
        bit_end = (st + 1) * 35
